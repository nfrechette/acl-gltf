...
%PYTHON_PATH% make.py -build -compiler %COMPILER% -config %CONFIG% -cpu %PLATFORM% -clean
IF NOT %ERRORLEVEL% EQU 0 GOTO :build_failure

%PYTHON_PATH% make.py -build -compiler %COMPILER% -config %CONFIG% -cpu %PLATFORM% -nosimd
IF NOT %ERRORLEVEL% EQU 0 GOTO :build_failure
